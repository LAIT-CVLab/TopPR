...
###########################################
def epi_KDE(data, position, h, kernel = "epanechinikov"):
...
# Confidence Band
###########################################
def confband_est(data, h, alpha = 0.1, kernel = "epanechinikov", p_val = True, repeat = 10):
    # data as numpy array
...
# BandWidth estimator
###########################################
def bandwidth_est(data, bandwidth_list = [], confidence_band = False, kernel = 'epanechnikov', alpha = 0.1, Plot = False):
    ...
    for h in tqdm(bandwidth_list):
        ...
        # filtration
        p_hat = epi_KDE(data, grid, h, kernel = kernel)
...
#   Top P&R
###########################################
def top_pr(real_features, fake_features, multiplier = 5, alpha = 0.1, kernel = "epanechinikov", random_proj = True, f1_score = True):
    ...
    # use balloon estimator to calculate Top P&R
        ...
        k = len(real_features[0]) * multiplier
    ...
    score_rg = epi_KDE(fake_features, real_features, bandwidth_f, kernel = kernel)
    inter_r = np.sum((score_rr > c_r) * (score_rg > c_g))

    # count significant real samples on fake manifold
    score_gr = epi_KDE(real_features, fake_features, bandwidth_r, kernel = kernel)
...
#   Outlier Detector
###########################################
def outlier_detector(features, k, proj = 32, alpha = 0.1, kernel = "epanechinikov"):
    ...
    if (features.size()[1] > 32):
        projection = torch.nn.Linear(features.size()[1], proj, bias = False).eval()
        torch.nn.init.kaiming_normal_(projection.weight)
        for param in projection.parameters():
            param.requires_grad_(False)
        features = projection(torch.Tensor(features)).detach().cpu().numpy()
    ...
    # estimation of confidence band and manifold
    cn, sample_density = confband_est(data = features, h = bandwidth, alpha = alpha, kernel = kernel, p_val = True)
    ...
    return dict(outlier_index = list(map(int, outlier)), density = sample_density, conf_band = cn, band = bandwidth)